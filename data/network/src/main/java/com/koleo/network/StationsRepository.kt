...
class StationsRepository @Inject constructor(
    ...
    suspend fun getStations(): NetworkResult<List<Station>?> = getResult(koleoApi.getStations())
    suspend fun getStationsKeywords(): NetworkResult<List<StationKeyword>?> =
        getResult(koleoApi.getStationsKeywords())

    private fun <T> getResult(response: Response<T>): NetworkResult<T?> {
        runCatching {
